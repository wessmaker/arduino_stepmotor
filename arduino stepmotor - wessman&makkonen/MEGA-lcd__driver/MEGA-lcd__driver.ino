...
void toDisplay(){
  ...
  if(keyPressed == timerONOFFButton){
    
    if (!timerOn){
      Serial.write(byte120);
      timerOn = true;
    }
     else if(timerOn){
      Serial.write(byte130);
      timerOn = false;
    }
...
void toTypeValue(){
  ...
  if(keyPressed == backButton){
    ...
    b = 0;
    c = 0;
    Serial.write(byte70);   //Peruuttaa ajan laittamisen
    Serial.write(byte70);
    Serial.write(byte70);
    changing = true;
  }
